...
def verilog_fetch(verilog_files, script_files):
    top_module = find_or_generate([sys.argv[1]], script_files, verilog_files)
    ...
    for pattern in [module_pattern, include_pattern]:
        ...
        for item in matches:
            if not item[0].startswith("module "):
                include_path = find_or_generate(item, script_files, verilog_files)
...
def find_or_generate(str_list, script_files, verilog_files):
    file_path = None
    ...
    if file_path is None:
        ...
        if script_path != "":
            ...
            generated_dir = move_to_generated_dir(script_path, current_directory)
...
def move_to_generated_dir(script_path, current_directory):
    verilog_extensions = [".v", ".vh", ".sv", ".svh", ".vs"]
    verilog_files_found = []
